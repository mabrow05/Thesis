...
\begin{equation}
  ...
  \left[ 1+ \langle P \rangle\cdot A(E)\beta\cos\theta \right],
\end{equation}
%
...
A_{\mathrm{SR}} = \langle P \rangle \cdot A(E) \cdot \frac{\beta}{2}.
\end{equation}
...
Rearrangment of the above equation provides a definition for $\Delta_i$:
%
...
For this analysis, we break the energy dependent systematic corrections into two categories:
$\Delta_{\mathrm{MC}}$ for Monte Carlo corrections and $\Delta_{\mathrm{Th}}$ for
corrections from theory for radiative and recoil order effects. The Monte Carlo
corrections can be further subdivided into individial contributions from
each event type, as defined in \ref{sec:backscattering}.
...
a corrections can be used to determine the value of the correction to be applied to the
data.
...
In \cite{UCNA2017}, $\Delta_{2}$ and $\Delta_{3}$ correspond to $\Delta_{\mathrm{Backsc}}$ and
...
\begin{figure}[h]
  ...
  \caption{Backscattering corrections for analysis choice C (All event types included with 2/3 separeated using
...
for the two detector sides. Another way to think of this is that once an event passes the
endcap, it's likelihood of triggering the detector is $\approx 100\%$
(*************************insert figure given to xuan),
so the thinner windows only affect events which do not make it to the detectors. Thus,
only events which trigger one detector are affected, the definition of a Type 0 event.
...
average value in a single bin due the spectrum not being flat. What I have described is
...
\Delta_3 = \frac{\frac{\beta}{2}}{\langle\beta\cos\theta\rangle}-1\textrm{ .}
\end{equation}
%
...
only a single correction $\delta_2$. New work in defining this correction allows for
...
\end{equation}
%
\noindent This is more difficult than separating $\Delta_2$ due to it not being a correction
that can be taken into account on an event by event basis. This requires evaluation of
...
determination of the asymmetry. As will be seen in \ref{sec:anaChoices}, one can include
...
the maximum fractional residual over energies of interest (with statistical power, so 180-700~keV),
we conservatively set the fractional discrapancy of our corrections. The values of 
...
out in a separate analysis by Eric Dees of North Carolina State University. The analysis
was finished prior to unblinding the asymmetry result so as to avoid any bias in the
values of the average polarization. The values for the $\pm$ spin-flipper states for
...
\begin{equation*} \label{eq:A_SR}
  ...
  \rangle \cdot A(E) \cdot \frac{\beta}{2}.
\end{equation*}
%
...
of the accuracy of the energy calibration at four discrete energies.
...
uncertainty envelope) is shown in figure (insert figure of energy uncertainty).
...
The raw measured spectra are highly contaminated with triggering events not from a
neutron $\beta$-decay within the decay trap. Such unwanted events include ambient
gamma ray events from neutron capture processes around the experimental hall, cosmic rays,
and other unforeseen events which appear as an electron.
Gamma ray events are highly suppressed due to the
requirement of a coincidence trigger between the MWPC and the scintillator (Add figure
of gamma suppression), and cosmic ray muon events are removed using a series of muon veto
...
a given run, and the subscipt data/bg determines whether the
...
(\ref{sssec:dataCuts}) used for asymmetry analysis. The reference spectra are essentially combinations
...
\begin{equation} \label{eq:bgRef}
  r_{\mathrm{bg},i} =  \frac{N_{\mathrm{bg},i}}{T_{\mathrm{bg}}}  \pm
  \frac{\sqrt{N_{\mathrm{bg,tot}} \frac{N_{\mathrm{ref,i}}}{N_{\mathrm{ref,tot}}} } }{T_{\mathrm{bg}}}
\end{equation}
%
...
Substantial work done previously by M. Mendenhall pioneered a robust determination
...
via normal background subtraction as the decay trap is void of neutrons during background
runs. The correction comes mainly from UCN which escape the decay trap
...
the scintillators ($n+^{1}\mathrm{H} \rightarrow ^{2}\mathrm{H}$). These would create excess events in the
background subtracted spectra
above the $\beta$-decay endpoint, a common characteristic in 2010 (used in
...
a background gamma ray (the MWPC is highly insensitive to gammas), electron events that fail to
...
energy in the MWPC, thus these events suffer the most from MWPC efficiencies $<100\%$. These same
higher energy, lower pitch angle events carry more asymmetry as seen by rewriting equation \ref{eq:A_SR}
...
the efficiency of our MWPCs has on our meaasured asymmetry. First we need to determine
...
The above efficiency is only valid at the energy of the $^{113}\mathrm{Sn}$ source, so instead one would
like to convert this into an energy threshold within the wirechamber.
Using an MWPC energy threshold rather than an efficiency
automatically creates an $E_{e^-}$ dependent efficiency, as the higher energy, lower pitch angle
...
the MWPC effiency in place, and then the correction is calculated in the usual way,
\begin{equation}
...
of the the gain within each run will present themselves as an additional
energy uncertainty. To study this effect, the individual endpoint values
of every run from the data was histogrammed and the $1\sigma$ spread in
...
\subsubsection{Magnetic Field Nonuniformity}
...
which shows that the position and shape alone of a field maxiumum or dip can create either
...
acceptance of the detected electrons, making electrons which never would have been detected before
...
reliable field data from 2011-2012, so a correction could have been properly calculated for
...
constants in the weak force lagrangian, $\lambda \equiv \frac{g_A}{g_V}$.
...
are completed using what we call blinded data, making this entire
analysis a blinded analysis. The idea behind blinded analyses
...
converges on a personally desired result. This could be an attempt to achieve agreement
...
can modify the energy and direction of the electron, therefore effecting the measured
...
any events that occur when the proton beam is out, which means no neutrons
...
in UCN Monitor 1 falls below 2~Hz. The rate for all data was generally $>20$~Hz.
Any events that occur within 0.05~s of the proton beam burst are also cut to improve
the signal to background ratio.

A few wirechamber cuts were also applied. The first is a cut on the shape of the wirechamber
...
like multiple positions in one wirechamber. The other wirechamber
...
of this analysis energy window is alotted its own section below, but each asymmetry shown
...
which consists of eight $\beta$-decay runs, eight background runs, and eight depolarization
runs, with the spin-state changing for each pair of runs so that there are
...
runs in the asymmetry), and pair asymmetries (using two consecutive spin-flipped runs). We use
...
\subsubsection{Analysis Choices}
...
trigger on detector package only, and they make up almost 95\% of detected events. Types 1, 2, and 3
...
indicating that Monte Carlo corrections may show improvement. The uncertainties in the figure are
purely statistical also, so the agreement seen is a worst case scenario.
...
\caption{Statistical and systematic errors used in minimization of the energy window. This is a projection
  of the \ref{fig:enWindow} about the minimum window cut of 190~keV to show the dependence on energy cut
  more effectively.}
...
\caption{Ratio of the asymmetry for every possible energy window divided by the
  ...
  consistent asymmetries covering the bottom right hand corner. When comparing
...
The final energy window is calculated by scanning all possible energy windows with a minimum
$>100$~keV and a maximum $<780$~keV. The width is set to start at 100~keV to save on computation
time. Upon exploring all energy windows, the minimum combined uncertainty
occurs at $190\mathrm{~keV} < E_e < 740\mathrm{~keV}$.
...
It is shown that the asymmetry is stable within $\pm0.3\%$ upon changing the analysis window
by roughly 100~keV in either direction about the minimum window, as evidenced by the uniform
area in the lower right hand corner of the plot.
...
similar weighting method as above, where all systematic uncertainties were
set to the smallest reported value between the
two measurements
and treated as completely correlated so as to avoid artificially small combined
systematic uncertainties. 
We obtain the values $A_0=-0.12015(34)_{\mathrm{stat}}(63)_{\mathrm{syst}}$ 
...
We can also report a combined result using the result from this analysis and our previous UCNA result
\cite{mendenhall2013} using a similar method as above, only now we set all systematic uncertainties
...
the $\Delta_3$, or $\cos\theta$ correction, which can be achieved by decreasing the fraction
of electrons that are lost due to inefficiencies within the spectrometer. These mainly come from
the existence of the foils at the end of the decay trap, so the idea is to remove the foils. With
...
As for the energy reconstruction, detectors with better linearity and perhaps built in position
reconstruction would be useful. T
