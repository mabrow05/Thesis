...
through once and the Type 3 passing through twice.



\subsubsection{Monte Carlo Study}
Monte Carlo studies provide
means for applying the most efficient cuts on the energy deposition in the wirechamber
to separate the two backscattering events as the true intitial direction of each event
is known and thus we de facto know the true type of the event. An example of the
wirechamber energy deposition for the two types of events can be seen in
Figure \ref{fig:two&threeDist}.The method goes as
follows

so we must rely on other
means to disentangle them. 
