...
The PMT resolution factors are key inputs into the detector response model, and they are especially
...
the conversion peak being fit. This is especially true for the $^{207}$Bi source, as it has two conversion
electron lines that appear as a convolution of two Gaussian-like peaks as seen in Figure ...
...
Typical single PMT ADC spectra from the $^{137}\mathrm{Ce}$, $^{113}\mathrm{Sn}$, and $^{207}\mathrm{Bi}$ sources can be
found in Figure \ref{fig:ADC_source_spectra}. Figure \ref{fig:En_source_spectra} show the
corresponding simulation spectra for the same PMT
with the detector response model applied. 
...
resolution factors. Figure \ref{fig:ADC_source_spectra} also shows the fits to each spectrum.

\subsubsection{Fitting the PMT response}
...
the full detector response model applied). Figure \ref{fig:MC2DataComp} shows the typical agreement for each source.
